# Claude Code 插件调研报告 - 2026年3月 (补充-第四期 v2)
...
| **开发者工具** | developer-kit | ⭐NEW | 多技术栈覆盖 |
...
| **反检测备用** | playwright-undetected | ⭐4 | 备用方案 |
...
- **Sailing Orders (航行命令)**: 任务分配
- **Battle Plans (战斗计划)**: 复杂任务规划
- **Action Stations (行动站)**: 职责分配
- **Captain's Log (船长日志)**: 进度追踪
...
- **目录范围**: 按目录作用域记忆
- **Token 优化**: 减少 50% token 使用
- **上下文管理**: 智能上下文切换
- **持久化**: 长期记忆存储

### 4.4 LangGraph 代理技能 ⭐ NEW
...
| 项目 | 说明 |
|-----|------|
| **GitHub** | [OutlineDriven/odin-claude-plugin](https://github.com/OutlineDriven/odin-claude-plugin) |
| **更新** | 3月2日 |
| **特点** | 大纲驱动的智能开发方法 |

#### 核心概念

- **Outline Driven**: 大纲优先开发
- **AST 分析**: 抽象语法树分析
- **上下文工程**: 现代化 CLI 工具
- **Batteries Included**: 开箱即用

### 4.6 Composio 插件集合 (⭐ 推荐)
...
| 方向 | 推荐技能 | Star | 特点 |
|-----|---------|------|------|
| **团队协调** | nelson | ⭐NEW | 海军指挥结构 |
| **项目管理** | jira-skill | ⭐NEW | JIRA 集成 |
| **记忆系统** | ham | ⭐NEW | 50% token 节省 |
| **AI 工作流** | mastering-langgraph | ⭐NEW | 状态化代理 |
| **开发方法** | odin-claude-plugin | ⭐NEW | 大纲驱动 |
...
```
Python Patterns + Pydantic AI + read-only-postgres + developer-kit
```

### 5.3 自动化测试推荐

```
Playwright + playwright-bot-bypass + test-writer-fixer + Claude Code Agents
```

### 5.4 团队协作推荐

```
Nelson (团队协调) + jira-skill (项目管理) + ham (记忆系统)
```

### 5.5 全栈开发推荐

```
Antigravity (970+ 技能) + Composio Plugins + developer-kit
```
...
3. **测试反检测**: playwright-bot-bypass 新增 Bot 绕过能力
4. **团队协作**: Nelson 海军指挥结构代理协调
5. **记忆优化**: HAM 减少 50% token 使用
6. **AI 工作流**: LangGraph 状态化代理技能
7. **开发方法**: ODIN 大纲驱动开发
...
- [Playwright Bot Bypass](https://github.com/greekr4/playwright-bot-bypass)
- [Nelson](https://github.com/harrymunro/nelson)
- [JIRA Skill](https://github.com/netresearch/jira-skill)
- [HAM](https://github.com/kromahlusenii-ops/ham)
- [LangGraph Agent Skill](https://github.com/SpillwaveSolutions/mastering-langgraph-agent-skill)
- [ODIN](https://github.com/OutlineDriven/odin-claude-plugin)
