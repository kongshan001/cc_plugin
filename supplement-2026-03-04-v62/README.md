# Claude Code 热门插件补充调研 (v62)

> 2026年3月 游戏/Python/测试/开发者工具热门插件完整调研 - 新增插件深度分析
...
|-----|-------------|------|------|
| Everything Claude Code | ⭐ 活跃 | 综合资源 | 8616+ 仓库索引 |
| Claude Code Agents | ⭐ 活跃 | E2E 开发 | 多审计员并行 |
| Fullstack Dev Skills | ⭐ 活跃 | 全栈开发 | 65+ 专业技能 |
| cc-devops-skills | ⭐ 活跃 | DevOps | IaC 代码生成 |
| Context Engineering Kit | ⭐ 活跃 | 上下文工程 | 最小 token 足迹 |
| Claude Code PM | ⭐ 活跃 | 项目管理 | 专业项目工作流 |
| Codex Skill | ⭐ 活跃 | 跨代理 | Codex 集成 |
| read-only-postgres | ⭐ 活跃 | 数据库 | 安全只读查询 |
| Web Assets Generator | ⭐ 活跃 | 前端资源 | 图标/OG 图像生成 |
| Book Factory | ⭐ 活跃 | 出版 | 非电子书写作 |
...
```bash
git clone httpsydantic-ai://github.com/p/skills ~/.claude/plugins/
```
...
### 5.12 Everything Claude Code

> 8616+ 仓库的综合资源 - Claude Code 百科全书

#### 背景需求

开发者需要一个全面的资源索引来发现 Claude Code 相关的项目和工具。
...
```bash
git clone https://github.com/affaan-m/everything-claude-code ~/.claude/plugins/
```

---

### 5.13 cc-devops-skills

> DevOps 工程师技能集 - IaC 代码生成

#### 背景需求

DevOps 工程师需要自动化工具来生成高质量的基础设施代码。
...
### 5.14 Context Engineering Kit

> 上下文工程工具包 - 最小 token 足迹

#### 背景需求

需要在保持高质量输出的同时最小化 token 使用。
...
```bash
git clone https://github.com/automazeio/ccpm ~/.claude/plugins/
```

---

### 5.16 Web Assets Generator
...
```bash
git clone https://github.com/alonw0/web-asset-generator ~/.claude/plugins/
```
...
| Dippy | ⭐⭐⭐⭐ | 命令安全 |
| Claude-tmux | ⭐⭐⭐⭐ | 会话管理 |
| Everything Claude Code | ⭐⭐⭐⭐⭐ | 综合资源 |
| cc-devops-skills | ⭐⭐⭐⭐ | DevOps |
| Context Engineering Kit | ⭐⭐⭐⭐ | 上下文优化 |
| Claude Code PM | ⭐⭐⭐⭐ | 项目管理 |
| Web Assets Generator | ⭐⭐⭐⭐ | 前端资源 |
...
6. **新增资源**：Everything Claude Code 提供了 8616+ 仓库的全面索引
